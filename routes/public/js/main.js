...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // ===== Language dropdown (works for both topbar and header instances) =====
  ['topbarLang', 'headerLang'].forEach(function(rootId) {
    const root = document.getElementById(rootId);
    if (!root) return;
    const isTopbar = rootId === 'topbarLang';
    const togSel = isTopbar ? '.topbar-lang-toggle' : '.header-lang-toggle';
    const optSel = isTopbar ? '.topbar-lang-option' : '.header-lang-option';
    const codeId = isTopbar ? 'topbarLangCode' : 'headerLangCode';
    const flagId = isTopbar ? null : 'headerLangFlag';
    const flagSmSel = isTopbar ? null : '.header-lang-flag-sm';

    const toggle = root.querySelector(togSel);
    const codeEl = document.getElementById(codeId);
    const flagEl = flagId ? document.getElementById(flagId) : null;
    const options = root.querySelectorAll(optSel);

    function syncCurrent(lang) {
      const map = { th: 'TH', en: 'EN', 'zh-CN': 'ZH', ja: 'JA' };
      if (codeEl) codeEl.textContent = map[lang] || lang.toUpperCase();
      if (flagEl && flagSmSel) {
        const matching = root.querySelector(optSel + '[data-lang="' + lang + '"] ' + flagSmSel);
        if (matching) {
          flagEl.innerHTML = matching.innerHTML;
          const vb = matching.getAttribute('viewBox');
          if (vb) flagEl.setAttribute('viewBox', vb);
        }
      }
    ...
    if (cookie) {
      ...
      if (lang && lang !== 'th') syncCurrent(lang);
    }
