...
router.post('/menus/:id', async (req, res, next) => {
  try {
    const { label, label_en, label_zh, label_ja, url, open_new_tab, active } = req.body;
    await query(
      'UPDATE menus SET label=?, label_en=?, label_zh=?, label_ja=?, url=?, open_new_tab=?, active=? WHERE id=?',
      [label, label_en || null, label_zh || null, label_ja || null, url, open_new_tab ? 1 : 0, active ? 1 : 0, req.params.id]
    );
    req.flash('success', 'อัปเดตเรียบร้อย');
    res.redirect(`${ADMIN_BASE}/menus`);
  } catch (err) { next(err); }
...
function translateWithMyMemory(text, from, to) {
  return new Promise((resolve, reject) => {
    ...
    const translateChunk = (chunk) => new Promise((res) => {
      ...
      const r = https.get(url, { timeout: 12000 }, (response) => {
        ...
        response.on('end', () => {
          try {
            ...
            } else if (json?.responseStatus === 429 || (json?.responseDetails || '').toLowerCase().includes('quota')) {
              anyFailed = true;
              console.warn('[mymemory] rate limited:', json.responseDetails || data.substring(0,100));
              res(chunk);
            } else if (json?.responseData?.translatedText) {
              // Some responses have status not 200 but translation present
              res(json.responseData.translatedText);
            } else {
              anyFailed = true;
              console.warn('[mymemory] no translation:', data.substring(0,150));
              res(chunk);
            }
          } catch (e) {
            anyFailed = true;
            console.warn('[mymemory] parse error:', e.message);
            res(chunk);
          }
        });
      });
      r.on('error', (e) => {
        anyFailed = true;
        console.warn('[mymemory] request error:', e.message);
        res(chunk);
      });
      r.on('timeout', () => {
        r.destroy();
        anyFailed = true;
        console.warn('[mymemory] timeout');
        res(chunk);
      });
    });
    ...
    Promise.all(chunks.map(translateChunk))
      .then(parts => {
        ...
        if (anyFailed) {
          // Reject so caller can show an error message
          return reject(new Error('MyMemory partial failure - some chunks could not be translated. Switch to Claude AI in Settings for reliable translation.'));
        }
        resolve(result);
      })
...
async function translateText(text, from = 'th', to = 'en') {
  ...
  // Try the configured premium provider first
  try {
  ...
  // MyMemory fallback - throws on partial failure so caller can decide
...
router.post('/translation-manager/translate-all', async (req, res, next) => {
  try {
    ...
    for (const n of newsList) {
      try {
        ...
        for (const [src, dest] of newsFieldMap) {
          if (n[src] && String(n[src]).trim()) {
            try {
              const result = await translateText(String(n[src]), 'th', lang);
              if (result && result !== n[src]) {
                updates[dest] = result;
              } else if (result === n[src]) {
                // Translation returned same as input - likely failed silently
                fieldErrors++;
                console.warn(`[translate-all] news ${n.id} field ${src}: result == input (likely failed)`);
              }
            } catch (fieldErr) {
              fieldErrors++;
              console.warn(`[translate-all] news ${n.id} field ${src} error:`, fieldErr.message);
            }
          }
        }
        if (Object.keys(updates).length > 0) {
          ...
          if (fieldErrors > 0) {
            console.warn(`[translate-all] news ${n.id}: ${Object.keys(updates).length} fields saved, ${fieldErrors} failed`);
          }
        } else {
          console.warn(`[translate-all] news ${n.id}: NO fields could be translated`);
          stats.errors++;
        }
      } catch (err) {
    ...
    for (const b of bizList) {
      try {
        ...
        for (const [src, dest] of bizFieldMap) {
          if (b[src] && String(b[src]).trim()) {
            try {
              const result = await translateText(String(b[src]), 'th', lang);
              if (result && result !== b[src]) {
                updates[dest] = result;
              } else if (result === b[src]) {
                fieldErrors++;
              }
            } catch (fieldErr) {
              fieldErrors++;
              console.warn(`[translate-all] business ${b.id} field ${src} error:`, fieldErr.message);
            }
          }
        }
        if (Object.keys(updates).length > 0) {
    ...
    // Menus
    stats.menus_translated = 0;
    stats.menus_skipped = 0;
    const labelField = `label_${lang}`;
    try {
      const menuList = await query('SELECT id, label, label_en, label_zh, label_ja FROM menus WHERE active = 1');
      for (const m of menuList) {
        try {
          if (!force && m[labelField] && String(m[labelField]).trim()) {
            stats.menus_skipped++;
            continue;
          }
          if (m.label && String(m.label).trim()) {
            const t = await translateText(String(m.label), 'th', lang);
            await query(`UPDATE menus SET ${labelField} = ? WHERE id = ?`, [t, m.id]);
            stats.menus_translated++;
          }
        } catch (err) {
          console.error('Menu translate error for id', m.id, ':', err.message);
          stats.errors++;
        }
      }
    } catch (err) {
      console.error('Menu list query error:', err.message);
    }

    const langLabel = { en: 'English', zh: 'Chinese', ja: 'Japanese' }[lang];
    let msg = `Translated to ${langLabel}: ${stats.news_translated} news + ${stats.businesses_translated} businesses + ${stats.menus_translated} menu items.`;
    if (stats.news_skipped + stats.businesses_skipped + stats.menus_skipped > 0) {
      msg += ` Skipped (already translated, use Force re-translate to overwrite): ${stats.news_skipped + stats.businesses_skipped + stats.menus_skipped}.`;
    }
    if (stats.errors > 0) {
      msg += ` ⚠ Failed: ${stats.errors}. The MyMemory free API has likely hit its rate limit. Configure Claude AI in Settings for reliable translation of long content.`;
    }
...
// ============================================================================
// v47: APP RESTART (Passenger restart.txt convention + auto-watch)
// ============================================================================
// Passenger watches the file at <app_root>/tmp/restart.txt and gracefully
// restarts the Node process when its modification time changes.
// We expose a button + route so admins can trigger this from the UI,
// AND a watcher that touches the file when key code/template files change
// (so re-uploading any of them auto-restarts without needing the button).
// ============================================================================
const RESTART_FILE = path.join(__dirname, '..', 'tmp', 'restart.txt');

function touchRestartFile() {
  try {
    const dir = path.dirname(RESTART_FILE);
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    const now = new Date();
    fs.writeFileSync(RESTART_FILE, `restart ${now.toISOString()}\n`);
    console.log('[restart] tmp/restart.txt touched at', now.toISOString());
    return true;
  } catch (e) {
    console.error('[restart] failed to touch:', e.message);
    return false;
  }
}

// Manual button: POST /<adminSlug>/restart-app
router.post('/restart-app', (req, res) => {
  const ok = touchRestartFile();
  if (ok) {
    req.flash('success', '✓ Application is restarting. New requests will hit the new version within a few seconds.');
  } else {
    req.flash('error', 'Failed to trigger restart. Please use Plesk Node.js panel as fallback.');
  }
  res.redirect(`${ADMIN_BASE}/dashboard`);
});

// Optional: GET ping that returns server PID + start time so admins can verify
// the restart actually happened (PID changes after Passenger restarts).
const _serverStartedAt = new Date();
router.get('/restart-app/status', (req, res) => {
  res.json({
    pid: process.pid,
    started_at: _serverStartedAt.toISOString(),
    uptime_seconds: Math.round(process.uptime()),
    restart_file_exists: fs.existsSync(RESTART_FILE),
    restart_file_mtime: fs.existsSync(RESTART_FILE) ? fs.statSync(RESTART_FILE).mtime.toISOString() : null
  });
});
...
// ----------------------------------------------------------------------------
// Watches the working directories you actually upload to. When any file
// inside changes (mtime updates due to FTP/Plesk re-upload), we touch
// tmp/restart.txt - Passenger picks up the change and restarts.
//
// We debounce by 2 seconds to avoid rapid-fire restarts during a multi-file
// upload (e.g. zip extract). The watcher only runs if AUTO_RESTART_ON_UPLOAD
// is enabled in settings (default: enabled).
// ----------------------------------------------------------------------------
const WATCH_PATHS = [
  path.join(__dirname),                                  // routes/
  path.join(__dirname, '..', 'views'),                   // views/
  path.join(__dirname, '..', 'public', 'css'),           // public/css/
  path.join(__dirname, '..', 'public', 'js'),            // public/js/
  path.join(__dirname, '..', 'middleware'),              // middleware/
  path.join(__dirname, '..', 'utils'),                   // utils/
  path.join(__dirname, '..', 'config'),                  // config/
  path.join(__dirname, '..', 'app.js')                   // app.js
];

let _restartTimer = null;
let _autoWatchEnabled = true;

function debouncedRestart(filename) {
  if (_restartTimer) clearTimeout(_restartTimer);
  _restartTimer = setTimeout(() => {
    if (!_autoWatchEnabled) return;
    console.log('[auto-restart] file changed:', filename, '- triggering restart');
    touchRestartFile();
    _restartTimer = null;
  }, 2000);
}

function setupAutoWatch() {
  for (const watchPath of WATCH_PATHS) {
    if (!fs.existsSync(watchPath)) {
      console.warn('[auto-restart] skipping non-existent path:', watchPath);
      continue;
    }
    try {
      fs.watch(watchPath, { recursive: true, persistent: false }, (eventType, filename) => {
        if (!filename) return;
        // Ignore noise: hidden files, node_modules, log files, the restart.txt itself
        if (filename.startsWith('.') ||
            filename.includes('node_modules') ||
            filename.endsWith('.log') ||
            filename.endsWith('restart.txt') ||
            filename.endsWith('.swp') ||
            filename.endsWith('~')) return;
        debouncedRestart(filename);
      });
      console.log('[auto-restart] watching', watchPath);
    } catch (e) {
      console.warn('[auto-restart] could not watch', watchPath, ':', e.message);
    }
  }
}

// Defer setup until after module load so the server has time to fully boot.
// Disable via env var if needed: AUTO_RESTART_DISABLED=1
if (!process.env.AUTO_RESTART_DISABLED) {
  setTimeout(setupAutoWatch, 5000);
} else {
  console.log('[auto-restart] disabled via AUTO_RESTART_DISABLED env var');
}
